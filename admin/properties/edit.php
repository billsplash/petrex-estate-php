...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($location) || empty($state) || $price <= 0) {
        ...
        $stmt = $db->prepare("UPDATE properties SET title=?, description=?, price=?, location=?, state=?, bedrooms=?, bathrooms=?, toilets=?, size=?, type=?, status=?, featured=?, image_url=?, agent_id=? WHERE id=?");
        $stmt->bind_param('ssdssiiidssisii', $title, $description, $price, $location, $state, $bedrooms, $bathrooms, $toilets, $size, $type, $status, $featured, $imageUrl, $agentId, $id);
    ...
    // Update $property for form re-display
    ...
    <main class="flex-1 p-8">
        <div class="flex justify-between items-center mb-8">
            <div>
                ...
                <p class="text-gray-500">Update property listing</p>
            </div>
            ...
            <form method="POST" action="?id=<?php echo (int)$id; ?>" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary"><?php echo htmlspecialchars($property['description'] ?? ''); ?></textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Price (₦) *</label>
                    <input type="number" name="price" required value="<?php echo htmlspecialchars($property['price']); ?>" min="0" step="0.01"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="state" required class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                        ...
                        <option value="<?php echo htmlspecialchars($s); ?>" <?php echo $property['state'] === $s ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="number" name="bedrooms" value="<?php echo (int)$property['bedrooms']; ?>" min="0"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Bathrooms</label>
                    <input type="number" name="bathrooms" value="<?php echo (int)$property['bathrooms']; ?>" min="0"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Toilets</label>
                    <input type="number" name="toilets" value="<?php echo (int)$property['toilets']; ?>" min="0"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Size (sqm)</label>
                    <input type="number" name="size" value="<?php echo htmlspecialchars($property['size'] ?? ''); ?>" min="0" step="0.01"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    ...
                    <select name="agent_id" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary">
                        ...
                        <option value="<?php echo (int)$agent['id']; ?>" <?php echo $property['agent_id'] == $agent['id'] ? 'selected' : ''; ?>>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="url" name="image_url" value="<?php echo htmlspecialchars($property['image_url'] ?? ''); ?>"
                        class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary"
                        placeholder="https://example.com/image.jpg">
                </div>
                <div class="md:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="featured" <?php echo $property['featured'] ? 'checked' : ''; ?> class="w-5 h-5 rounded">
